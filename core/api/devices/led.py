    ...
    def apply_color(self,color,delay):
        ...
        if color == self.curr_color :
            ...
            for i in range(len(r_list)) :
                tini = time.time()
                command = ''
                command += f"echo {self.pins['r']}={self.imax*r_list[i]/255*100}% > /dev/servoblaster;"
                command += f"echo {self.pins['g']}={self.imax*g_list[i]/255*100}% > /dev/servoblaster;"
                command += f"echo {self.pins['b']}={self.imax*b_list[i]/255*100}% > /dev/servoblaster;"
                os.system(command)
